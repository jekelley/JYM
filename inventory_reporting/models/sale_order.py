...
class SaleOrder(models.Model):
    ...
    def get_open_order(self):
        ...
        data_list = []
        order_ids = self.search([('state', 'in', ['draft', 'sent'])])
        for order in order_ids:
            ...
            for line in order.order_line:
                ...
                report_data_list.append(
                    {'name': line.product_id.name, 'description': line.name,
    ...
    def print_excel_report(self):
        ...
        if data_list:
            for data in data_list:
                ...
                worksheet.write(row, col, 'Sale Order Number', header_format)
                worksheet.write(row, col + 1, data['order'].name,
                                header_format)
                row += 1
